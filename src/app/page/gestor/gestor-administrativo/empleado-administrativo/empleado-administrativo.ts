...
@Component({
  selector: 'app-empleado-administrativo',
  imports: [
    CommonModule,
    ReactiveFormsModule
  ],
  templateUrl: './empleado-administrativo.html',
  styleUrl: './empleado-administrativo.css',
})
export class EmpleadoAdministrativo implements OnInit, OnDestroy {
...
    // Paso 2: Aplicar los valores (incluyendo horas_administrativas y horas_academicas)
...
      // Se mantiene el orden de Obrero (primero grado, luego cargo)
...
    // Paso 3: Reconfigurar validadores y deshabilitar los campos que no correspondan al tipo de personal.
    const tipoPersonalId = this.formularioEmpleado.get('id_tipo_personal')?.value;
    if (tipoPersonalId !== null) {
      this.configurePersonalSpecificFields(tipoPersonalId);
    }

    // Paso 4: Reaplicar el filtro de cargos de obrero y setear el cargo correcto.
    if (empleado.id_tipo_personal === 4 && empleado.id_grado_obrero) {
      this.filtrarCargosObrero(empleado.id_grado_obrero);
      // Se usa setTimeout para asegurar que los cargos filtrados se hayan cargado antes de setear el valor.
      setTimeout(() => {
